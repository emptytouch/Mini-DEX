...
本文件按 Task 7 作业要求的条目顺序编排，每一条都附可复现的命令或可在 Snowtrace 上核对的链上数据。
...
| # | 要求 | 交付内容 | 在哪 |
|:--:|---|---|---|
| 1 | 代码仓库或提交记录 | 仓库 + 按功能拆分的提交 | 见 §六 |
| 2 | `npm test` 与 `forge test` 全绿的证明 | `npm test` **61 passed / 7 files**；`forge test` **22 passed** | §1.1 / §2.1 |
| 3 | 3 个 Fuji 合约地址 | Vault / USDC / WAVAX | §2.2 |
| 4 | deposit 与 withdraw 的交易哈希 | deposit `0x288b5139…`；withdraw `0x14bc8b70…` | §2.3 / §3.4 |
| 5 | 登录、余额、两地址成交的截图 | 操作步骤见 RUNBOOK §6–§8 | §3.1–§3.3 |
| 6 | 所选进阶功能的代码、测试或演示证明 | 6 项，各有代码 + 测试 + 演示证据 | §四 |
...
```
test(server): 补充时间优先与 self-trade 用例，修复整档自持中断撮合
feat(contracts): Vault 增加单笔提现限额与金库偿付校验
feat(server): 支持 IOC / FOK 订单
feat(server): 做市模块，镜像 Binance 盘口
feat(server): WebSocket 私有 orders 频道
feat(server): 状态落 SQLite，重启不丢
fix(server): 重启回放重复扣款 —— 用 nonce 认领已实时扣账的提现
docs: 交付说明、操作手册、安全审查报告、功能完善说明
```
